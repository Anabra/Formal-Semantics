...
Inductive step : aexp * state -> aexp * state -> Prop :=
  ...
  | fplusl (a1 a2 : aexp)(s : state)(i : nat) :
           fstep (a1 , s) i ->
           step (APlus a1 a2, s) (APlus (ANum i) a2, s)
  | plusr (a2 a2' : aexp)(s : state)(n : nat) :
          step (a2 , s) (a2' , s) ->
          step (APlus (ANum n) a2 , s) (APlus (ANum n) a2' , s).
...
Proof.
  apply (plusl _ _ _ st).
  apply (fplusl _ _ st 12).
  apply (var Y).
Qed.
...
Proof.
  ...
  induction p.
    intros. inversion p'. trivial.
    intros. inversion p'. trivial.
    intros. inversion p'. rewrite (IHp _ H3). trivial.
Qed.
...
Proof.
  apply (ftranst _ (17 +' 5, st)).
    apply (transt _ (17 +' Z, st)).
      apply (pluslt). apply ().
Qed.
